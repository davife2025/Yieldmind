...
// ─────────────────────────────────────────────────────────────
// AlertsView — Full-page alerts with severity filter
// Uses useRealtimeAlerts for live data (same source as TopBar badge)
// ─────────────────────────────────────────────────────────────
...
export function AlertsView() {
  ...
  // Live data from Supabase Realtime — same source as the TopBar bell badge
  ...
  const filtered = alerts.filter((a: RiskAlert) =>
    filter === "ALL" || a.severity === filter
  )
  const active   = alerts.filter((a: RiskAlert) => !a.resolved)
  const resolved = alerts.filter((a: RiskAlert) => a.resolved)
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          ...
          { label: "High+",     value: active.filter((a: RiskAlert) => ["CRITICAL","HIGH"].includes(a.severity)).length, color: "text-danger",    bg: "bg-danger/5   border-danger/20"   },
      ...
      <Card>
        <SectionHeader
          title="Alert Feed"
          subtitle="AI-detected risk signals · live from Supabase Realtime"
          icon={<ShieldAlert className="w-4 h-4" />}
          action={
            <div className="flex items-center gap-2">
              {connected ? <LiveIndicator /> : <span className="text-xs text-text-muted">Connecting...</span>}
              <div className="flex bg-surface-muted rounded-lg p-0.5 ml-1">
        ...
        {filtered.length === 0 ? (
          <EmptyState
            ...
            title={alerts.length === 0 ? "No alerts yet" : "No alerts match this filter"}
            description={alerts.length === 0
              ? "The AI agent will surface risk signals here as it monitors your portfolio."
              : "All clear for this severity level."
            }
          />
        ) : (
          <div className="space-y-3">
            {filtered.map((alert: RiskAlert) => {
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        {alert.asset_id && <Badge variant="neutral">{alert.asset_id}</Badge>}
